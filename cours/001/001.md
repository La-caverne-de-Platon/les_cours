...
# EXERCICES
```info
Lisez le passage proposé et effectuez les tâches indiquées ci-dessous ; vous pourrez ainsi commencer à tester vos compétences logiques dans l'interprétation de textes et de documents écrits :
```
...
```info
Certains mots ont été supprimés du texte de Calvino ; identifiez la "série" qui les contient dans le bon ordre :
```
...
```info
indiquez laquelle des affirmations suivantes peut être déduite logiquement du passage :
```
